...
def ensure_initial_admin(config):
    ...
    def _ensure_initial_admin(config):
        ...
        if not (config('identity-backend') ==
            ...
            if get_api_version() > 2:
                passwd = create_user_credentials(config('admin-user'),
                                                 get_admin_passwd,
                                                 set_admin_passwd,
                                                 domain=ADMIN_DOMAIN)
                if passwd:
                ...
                create_user_credentials(config('admin-user'), get_admin_passwd,
...
def create_service_credentials(user, new_roles=None):
    ...
    return create_user_credentials(user, get_service_password,
